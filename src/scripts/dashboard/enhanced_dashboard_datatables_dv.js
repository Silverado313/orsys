...
function loadVouchers() {
    ...
    db.collection("dr.vouchers").orderBy("entryDate", "desc")
        .get()
        .then((querySnapshot) => {
                ...
                render: function(data, type, row) {
                    return `
                        <div class="action-buttons">
                            ...
                            <button class="btn btn-sm btn-outline-warning edit-view" onclick='window.open("/src/pages/admin/dv/edit.html?slip=${row.slipNo}", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=150,left=250,width=800,height=600")';>
...
function addCustomFeatures() {
    ...
    if (tableContainer && !document.getElementById('totals-display')) {
        const totalsHtml = `
            <div id="totals-display" class="row mb-3">
                <div class="col-md-12">
                    <div class="card bg-light">
                        <div class="card-body py-2">
                            <div class="row text-center">
                                <div class="col-md-3">
                                    <h6 class="mb-1 text-muted">Total Vouchers</h6>
                                    <h4 class="mb-0 text-primary" id="total-vouchers">0</h4>
                                </div>
                                <div class="col-md-3">
                                    <h6 class="mb-1 text-muted">Total Amount</h6>
                                    <h4 class="mb-0 text-success" id="total-amount">PKR 0</h4>
                                </div>
                                <div class="col-md-3">
                                    <h6 class="mb-1 text-muted">Completed</h6>
                                    <h4 class="mb-0 text-success" id="completed-amount">PKR 0</h4>
                                </div>
                                <div class="col-md-3">
                                    <h6 class="mb-1 text-muted">Pending</h6>
                                    <h4 class="mb-0 text-warning" id="pending-amount">PKR 0</h4>
                                </div>
                            </div>
                        </div>
                    </div>
...
function exportToCSV() {
    ...
    let csvContent = "Slip No, Payment From, Head, Point Person, Payment Mode, Status, Total Amount,Date,Remarks\n";
...
function attachEventListeners() {
    ...
    document.querySelectorAll('.print-receipt').forEach(button => {
        button.addEventListener('click', function() {
            ...
            window.open(`/src/pages/dv/print.html?slip=${slipNo}`, "_blank", 
...
function deleteReceipt(receiptId) {
    ...
    if (confirm(`Are you sure you want to delete voucher ${voucher.slipNo}?\n\nThis action cannot be undone.`)) {
        ...
        db.collection("dr.vouchers").doc(receiptId).delete()
            .then(() => {
